...
חוקי מלאי — חשוב מאוד:
- מוצרים שנזרקים בסוף שבוע (לא מפחיתים מהם כלום — הם לא יהיו שם שבוע הבא):
  חלות מתוק, גביניות, פס שמרים גבינה
- מוצרים שמחזיקים לשבוע הבא (הפחת את כל המלאי הנשאר — הם יהיו על המדף גם שבוע הבא):
  רוגלך שוקולד, רוגלך עלים קקאו, קוקוש קייק, קראנץ' קקאו, פס שמרים קקאו שקית, פס שוקולד פירורים

לדוגמה: נשאר 4 יח' רוגלך שוקולד → הפחת 4 (כולם יהיו על המדף שבוע הבא).
נשאר 3 יח' גביניות → אל תפחית כלום (יוזרקו, לא יהיו שם שבוע הבא).

מצב השבוע הקרוב: {context_str}
{user_notes_section}
כללי חשיבה חשובים:
1. **עדיפות לשבועות האחרונים** — 4-6 השבועות האחרונים מייצגים את המציאות הנוכחית. אם מוצר = 0 בשבועות האחרונים — אל תמליץ עליו.
2. **זהה טרנדים** — אם מוצר יורד (12→6→3→0), המשך את הטרנד.
3. **אל תממצע עיוורת** — ממוצע 10 שבועות כולל שבועות ישנים. בחר שבועות דומים לאחרונה.
4. **חשב מלאי לפי הכללים למעלה** — שבועיים = מחצית, שבוע אחד = הכל.
5. **הערות בעל החנות** — משקל גבוה מאוד.

תהליך החשיבה שחייב לקרות לפני שאתה כותב את ה-JSON:
שלב א: קבע כמות ראשונית לכל מוצר לפי ההיסטוריה + מלאי.
שלב ב: חשב סה"כ עלות (כמות × מחיר קנייה לכל מוצר). רשום את החשבון בפירוט.
שלב ג: אם יש תקציב שצוין בהערות — בדוק האם עברת אותו. אם כן, הפחת כמות ממוצר אחד בכל פעם וחשב מחדש עד שעומד בתקציב.
שלב ד: רק עכשיו, אחרי שוידאת שהסה"כ עומד בתקציב — כתוב את ה-JSON עם הכמויות הסופיות.

⚠️ חשוב: ה-JSON חייב להכיל בדיוק את הכמויות שחישבת בשלב ג — לא את הכמויות מלפני ההפחתה!

ענה בפורמט הבא בלבד:
{{
  "המלצות": {{"שם_מוצר": כמות, ...}},
  "הסבר": "סיכום: מה הזמנת ולמה, כולל איך עמדת בתקציב"
}}"""
