...
חוקי מלאי — חשוב מאוד:
- מוצרים שנזרקים בסוף שבוע (הפחת את כל המלאי הנשאר — הם לא עוברים לשבוע הבא):
  חלות מתוק, גביניות, פס שמרים גבינה
- מוצרים שיכולים להחזיק לשבוע הבא (הפחת רק מחצית מהמלאי הנשאר):
  רוגלך שוקולד, רוגלך עלים קקאו, קוקוש קייק, קראנץ' קקאו, פס שמרים קקאו שקית, פס שוקולד פירורים

לדוגמה: נשאר 4 יח' רוגלך שוקולד → הפחת 2 (המחצית תעבור לשבוע הבא).
נשאר 3 יח' גביניות → הפחת 3 (יוזרקו בסוף שבוע).
...
חשוב מאוד: ה-JSON חייב להכיל את הכמויות הסופיות אחרי כל ההתאמות (כולל תקציב). אל תכתוב כמות ראשונית ואז תקצץ בהסבר — שים בJSON רק את מה שבאמת תזמין.

ענה ב-JSON בפורמט הבא בלבד:
{{
  "המלצות": {{"שם_מוצר": כמות, ...}},
  "הסבר": "משפט קצר — על מה התבססת + איך עמדת בתקציב אם נדרש"
}}"""
    ...
    def calculate_recommendation(
        ...
        for product in products:
            ...
            # מוצרים שנזרקים — הפחת הכל; מוצרים שמחזיקים — הפחת מחצית
