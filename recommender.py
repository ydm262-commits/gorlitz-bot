...
class OrderRecommender:
    ...
    def format_order_message(recommendations, week_date, summary, weather_desc="", holiday_desc=""):
        db = get_database()
        lines = ["ðŸ¥– ×”×–×ž× ×” ×ž-×’×¨\×™×¥", f"ðŸ“… ×œ×©×‘×•×¢ ×©×œ {week_date}", "", "ðŸ“¦ ×”×–×ž× ×” ×ž×•×ž×œ×¦×ª:"]
        total_cost = 0
        for product_name, qty in sorted(recommendations.items()):
            if qty > 0:
                ...
                total_cost += cost
                lines.append(f"  â€¢ {product_name}: {qty} ×™×—' (â‚ª{round(cost)})")
        lines.extend(["", f"ðŸ“Š", f"×¡×”×´×š ×”×–×ž× ×”: â‚ª{round(total_cost)}", "", "×™×¢× ×§×™, ×¡×–×‘ ÞP× × ×—×“×©×” ××ª ×”×ž×œ××™!ðŸ“±"])
        return "\n".join(lines)
