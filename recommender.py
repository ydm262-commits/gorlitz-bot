...
{user_notes_section}
כללי חשיבה חשובים:
1. **עדיפות לשבועות האחרונים** — 4-6 השבועות האחרונים בטבלה מייצגים את המציאות הנוכחית. אם מוצר מסוים מופיע כ-0 בשבועות האחרונים, כנראה הפסיקו להזמין אותו — אל תמליץ עליו.
2. **זהה טרנדים** — אם מוצר יורד (12→6→3→0), המשך את הטרנד. אם עולה, שקול להגדיל.
3. **אל תממצע עיוורת** — ממוצע של 10 שבועות כולל שבועות ישנים שלא רלוונטים. בחר שבועות דומים *לאחרונה*.
4. **מלאי קיים** — הפחת בדיוק את מה שנשאר (חוץ ממוצרים דו-שבועיים שמחצית מהם נספרת).
